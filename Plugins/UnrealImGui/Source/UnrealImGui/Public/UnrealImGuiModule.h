...
public:
    ...
    virtual void ClearEditorWindowImGuiDelegate(int32 index);
#endif
	...
	FSimpleMulticastDelegate OnImGuiWidgetConstruct;
	FSimpleMulticastDelegate OnImGuiWidgetUpdate;
	FSimpleMulticastDelegate OnImGuiWidgetDestroy;
private:
	TSharedPtr<SImGuiWidgetEd> ImGuiWidget;
	TSharedPtr<SEditableTextBox> SyncInputTextBox;
	ImGuiInputTextCallbackData* CurrentData = nullptr;
	...
	void OnClosePluginTab(TSharedRef<SDockTab>);

	void OnSyncInputTextBoxTextChanged(const FText& ChangedText);
