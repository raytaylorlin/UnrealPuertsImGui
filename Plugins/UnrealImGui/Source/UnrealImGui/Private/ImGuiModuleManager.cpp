...
void FImGuiModuleManager::LoadTextures()
{
	...
	if (!bTexturesLoaded)
	{
		...
		ImFontAtlas& FontAtlas = ContextManager.GetFontAtlas();

		FontAtlas.AddFontDefault();
		// Add font to support Chinese
		ImGuiIO& IO = ImGui::GetIO();
		FString ContentDir = IPluginManager::Get().FindPlugin(TEXT("UnrealImGui"))->GetBaseDir();
		FString FontPath = ContentDir + TEXT("/Resources/simhei.ttf");
		IO.Fonts->AddFontFromFileTTF(TCHAR_TO_ANSI(*FontPath), 14.0f, NULL, IO.Fonts->GetGlyphRangesChineseFull());
